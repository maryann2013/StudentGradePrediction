...
st.subheader("This tool provides 3 solutions \n 1.Business Analytics for Student performance \n  2.Batch Prediction \n  3.Single Prediction")
options = st.selectbox("Please Select", ['Select','Business Analytics for Student performance','Batch Prediction','Single Prediction'])

if (options =='Business Analytics for Student performance'):
    st.markdown("""
    ...
    if st.sidebar.button('Predict'):  
        ...
        st.write('Final Grade would be : ', np.round_((prediction[0]/20)*100),'%')
        ...
        if prediction[0] <10:
            ...
            for feat in key_features:
                ...
                if int(final_features_df[feat]) < int(dict_25[f25]):
                    studRisk='<p style="font-family:Courier; color:Black; font-size: small;">'+feat+' -> '+str(int(final_features_df[feat]))+' is below Class Avg:'+str(np.round(dict_mean[fmean],2))+'</p>'
                ...
                if int(final_features_df[feat]) > int(dict_75[f75]):
                    studRisk='<p style="font-family:Courier; color:Black; font-size: small;">'+feat+' -> '+str(int(final_features_df[feat]))+' is above Class Avg:'+str(np.round(dict_mean[fmean],2))+'</p>'
                    st.write(studRisk, unsafe_allow_html=True)
    ...
    if uploaded_file is not None:
        ...
        if st.sidebar.button('Predict'):  
            ...
            st.write("No Of Students in trouble: ",len(predicted_students_in_trouble))
           
            # See which feature they landed well below or well above peers
            for index, row in predicted_students_in_trouble.iterrows():
                studDet='<p style="font-family:Courier; color:Black; font-size: small;"><u>Student ID :'+str(index+1)+'</u></p>'
                ...
                for feat in key_features:
                    ...
                    if int(row_df[feat]) < student_por_df[feat].quantile(0.25):
                        studRisk='<p style="font-family:Courier; color:Black; font-size: small;">'+feat+' -> '+str(int(row_df[feat]))+' is below Class Avg:'+str(np.round(np.mean(student_por_df[feat]),2))+'</p>'
                        #st.write('\t', 'Below:', feat, int(row_df[feat]), 'Class:', np.round(np.mean(student_por_df[feat]),2))
                        st.write(studRisk, unsafe_allow_html=True)
                    if int(row_df[feat]) > student_por_df[feat].quantile(0.75):
                        studRisk='<p style="font-family:Courier; color:Black; font-size: small;">'+feat+' -> '+str(int(row_df[feat]))+' is above Class Avg:'+str(np.round(np.mean(student_por_df[feat]),2))+'</p>'
                        #st.write('\t','Above:', feat, int(row_df[feat]), 'Class:', np.round(np.mean(student_por_df[feat]),2))
                        st.write(studRisk, unsafe_allow_html=True)
