...
def load_data():
    df = pd.DataFrame({'SchoolMgmt': ['Government','Private']
                        'MotherEducation': ['None','Primary Education','5th to 9th Grade','Secondary Education','Higher Education'],
                       ...
                       'StudentGuardian':['Mother', 'Father','Other'],
                       ...
                       'Extra-curricularActivities' :['Yes','No'], 
...
SchoolMgmt=st.selectbox("School Type", df['SchoolMgmt'].unique())
...
MotherJob=st.selectbox("Mother's Job", df['MotherJob'].unique())
# converting text input to numeric to get back predictions from backend model.
...
FatherJob=st.selectbox("Father's Job", df['FatherJob'].unique())
# converting text input to numeric to get back predictions from backend model.
...
MotherEducation=st.selectbox("Mother's Education", df['MotherEducation'].unique())
# converting text input to numeric to get back predictions from backend model.
...
FatherEducation=st.selectbox("Father's Education", df['FatherEducation'].unique())
# converting text input to numeric to get back predictions from backend model.
...
ReasonForSchool=st.selectbox("Reason to choose this school", df['ReasonForSchool'].unique())
# converting text input to numeric to get back predictions from backend model.
...
StudentGuardian=st.selectbox("Student Guardian", df['StudentGuardian'].unique())
# converting text input to numeric to get back predictions from backend model.
...
ExtraEducationalSupport=st.selectbox("School providing Extra Educational Support", df['ExtraEducationalSupport'].unique())
# converting text input to numeric to get back predictions from backend model.
...
ExtraPaidClassesWithinTheCourseSubject=st.selectbox("Extra paid classes within the course subject", df['ExtraPaidClassesWithinTheCourseSubject'].unique())
# converting text input to numeric to get back predictions from backend model.
...
Extra-curricularActivities=st.selectbox("Extra-curricular activities", df['Extra-curricularActivities'].unique())
# converting text input to numeric to get back predictions from backend model.
if Extra-curricularActivities == 'Yes':
    activities_no = 0
    activities_yes=1
else:
    activities_no = 1
...
FamilyEducationalSupport=st.selectbox("Family providing Educational Support", df['FamilyEducationalSupport'].unique())
# converting text input to numeric to get back predictions from backend model.
...
TravelTime=st.selectbox("Time to travel to School", df['TravelTime'].unique())
# converting text input to numeric to get back predictions from backend model.
...
AttendedNurserySchool=st.selectbox("Attended Nursery School", df['AttendedNurserySchool'].unique())
# converting text input to numeric to get back predictions from backend model.
...
WillingToTakeHigherEducation=st.selectbox("Willing to take higher education", df['WillingToTakeHigherEducation'].unique())
# converting text input to numeric to get back predictions from backend model.
...
InternetAccessAtHome=st.selectbox("Internet Access At Home", df['InternetAccessAtHome'].unique())
# converting text input to numeric to get back predictions from backend model.
...
RomanticRelationship=st.selectbox("Romantic Relationship", df['RomanticRelationship'].unique())
# converting text input to numeric to get back predictions from backend model.
...
StudyTime=st.selectbox("Hours of Study", df['StudyTime'].unique())
# converting text input to numeric to get back predictions from backend model.
...
QualityOfFamilyRelationships=st.selectbox("Quality of family relationship", df['QualityOfFamilyRelationships'].unique())
...
GoingOutWithFriends=st.selectbox("Going Out With Friends", df['GoingOutWithFriends'].unique())
...
WorkdayAlcoholConsumption=st.selectbox("Workday Alcohol Consumption", df['WorkdayAlcoholConsumption'].unique())
...
WeekendAlcoholConsumption=st.selectbox("Weekend Alcohol Consumption", df['WeekendAlcoholConsumption'].unique())
...
CurrentHealthStatus=st.selectbox("Current Health Status", df['CurrentHealthStatus'].unique())
...
FreetimeAfterSchool=st.selectbox("Freetime after Schoolhours", df['FreetimeAfterSchool'].unique())
...
AddressType=st.selectbox("Address Type", df['AddressType'].unique())
# converting text input to numeric to get back predictions from backend model.
...
FamilySize=st.selectbox("Family Size", df['FamilySize'].unique())
# converting text input to numeric to get back predictions from backend model.
...
ParentCohabitationStatus=st.selectbox("Parent Cohabitation Status", df['ParentCohabitationStatus'].unique())
# converting text input to numeric to get back predictions from backend model.
